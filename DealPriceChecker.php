...
$count = 0;
if (time() - fileatime('cookie2.txt') > 82800)
{
	authorize();
}
foreach ($deals as $deal)
{
	...
	if ($deal['lat'] != 0 && $deal['lng'] !=0)
	{
		...
		foreach ($chosen_by_distance as $item) // where $item[0] is ad id
		{
			...
			if ($deal['rooms'] == $ad['rooms'] && $deal['floor'] == $ad['floor'])
			{
				...
				{
					if ($deal['price'] > $ad['price'])
					{
						if (isActive($item[0]))
						{
							$status = 2;
							$existsPriceLessCurrent = true;
							$url = 'https://www.cian.ru/sale/flat/' . $item[0];
							sleep(1);
							break;
						}
					}
	...
	$dbWorker->updateCrmDeal($deal_index, $status);
	$count++;
}
...
function isActive($id)
{
	...
	echo 'https://www.cian.ru/sale/flat/' . $id . '/'."<br/>";
